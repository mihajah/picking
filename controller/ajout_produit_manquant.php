<?php
	...
	$idproduit = (isset($_GET['idproduit']) && $_GET['idproduit'] > 0) ? $_GET['idproduit'] : 0;
	...
	if($idproduit > 0){
		...
		if(isset($_SESSION['NombrePickingProduit'][$idproduit])){
			$QuantiteProduit = $_SESSION['NombrePickingProduit'][$idproduit];
		}else{
			$QuantiteProduit = $Commandes['cart'][$idproduit];
		}

		$QuantiteProduit = $QuantiteProduit - 1;
		$_SESSION['NombrePickingProduit'][$idproduit] = $QuantiteProduit;
	...
	//On enregistre la liste des produits manquantes en session
	//On recupere les infos du produit
	$products = array();
	if($idproduit){
		$products = products($idproduit);
	}
	$_SESSION['ListeProduitManquant'][$IdCommandes][] = $products;
